...
def quotes(request):
    a = set(User_Quote.objects.all())
    b = set(User_Quote.objects.filter(my_favorites__created_by=request.session['user_email']))
...
def favequote(request, quote_id):
    ...
    if User.objects.filter(email =  request.session['user_email']) or Quote.objects.filter(id = quote_data) :
        messages.info(request,'You have already favorited this quote!')
        return redirect ('/quotes')
    else:
...
def remove_quote(request):
    User_Quote.objects.filter(my_favorites=request.session['quote_id']).filter(user_quote__email=request.session['user_email']).delete()
